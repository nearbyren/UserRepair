...
sealed class ResponseHolder<out T> {
    ...
    data class Success2<T>(val code: Int, var message: String? = null, val data: T?) : ResponseHolder<T>()
     ...
     * 处理网络请求成功
     */
    suspend fun onSuccess2(action: (suspend (code: Int, message: String?, data: T?) -> Unit)): ResponseHolder<T> {
        if (isSuccess2()) {
            onCompletion?.invoke()
            action.invoke((this as Success2).code, (this as Success2).message, (this as Success2).data)
        }
        return this
    }

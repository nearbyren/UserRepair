    ...
    override fun uncaughtException(t: Thread, e: Throwable) {
        ...
        if (!handleException(e) && mDefaultHandler != null) {
            ...
            try {
                ...
                println("CrashHandlerManager error")
            }
            println("CrashHandlerManager 结束程序")
            //结束程序
...
//        }.start()
    ...
    private fun sendCrashReportsToServer() {
        ...
        if (crFiles != null && crFiles.isNotEmpty()) {
            ...
            for (fileName in sortedFiles) {
                val cr = File(ApplicationProvider.appContext.filesDir, fileName)
                postReport(cr)
                //cr.delete()
            }
        }
    ...
    private fun getCrashReportFiles(): Array<String>? {
        val filesDir = ApplicationProvider.appContext.externalCacheDir
        filesDir?.let {
            val filter =
    ...
    private fun saveCrashInfoToFile(throwable: Throwable): String? {
        ...
        try {
            ...
            val path =
                ApplicationProvider.appContext.externalCacheDir?.absolutePath + File.separator + "crash_xy" //     /sdcard/crash/crash-time-timestamp.log
            val dirs = File(path)
            if (!dirs.exists()) {
                dirs.mkdirs()
            }
...
//                FileOutputStream trace = new FileOutputStream(file)
            ...
            println("CrashHandlerManager an error occured while writing file...$e")
        }
        return null
    }
    ...
    fun saveTrimMemoryInfoToFile(info: String?): String? {
        collectDeviceInfo()
        println("CrashHandlerManager saveTrimMemoryInfoToFile")
        ...
        try {
            ...
            val path =
                ApplicationProvider.appContext.externalCacheDir?.absolutePath + File.separator + "aCrash_xy" //     /sdcard/crash/crash-time-timestamp.log
            val dirs = File(path)
            if (!dirs.exists()) {
                dirs.mkdirs()
            }
    ...
    private fun collectDeviceInfo() {
        try {
            val packageManager = ApplicationProvider.appContext.packageManager
            ...
            println("CrashHandlerManager NameNotFoundException")
        }
